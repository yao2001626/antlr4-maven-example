...
public final class Builder {
    ...
    public static final class Lexer {
        ...
        public LimeLexer build() {
            return this.lexer;
        }
    }
    public static final class Parser {

        private LimeParser parser;
        ...
        public Parser(CharStream input) {
        	LimeLexer lexer = new LimeLexer(input);
            lexer.removeErrorListeners();
            lexer.addErrorListener(ERROR_LISTENER);
            this.parser = new LimeParser(new CommonTokenStream(lexer));
            this.parser.removeErrorListeners();
            this.parser.addErrorListener(ERROR_LISTENER);
        }

        public Parser(LimeLexer lexer) {
            this.parser = new LimeParser(new CommonTokenStream(lexer));
            this.parser.removeErrorListeners();
            this.parser.addErrorListener(ERROR_LISTENER);
        }
    ...
    public static final class Tree {
        ...
        public String toStringASCII() {
        	...
        	LimeParser parser = new Builder.Parser(input).build();
            ...
            while (!childListStack.isEmpty()) {
                ...
                if (childStack.isEmpty()) {
                    ...
                    if (tree instanceof TerminalNode && tree.getText().trim().isEmpty()) {
                        text = "<" + LimeParser.tokenNames[((TerminalNode)tree).getSymbol().getType()] + ">";
                    } else {
