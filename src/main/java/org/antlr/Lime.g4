...
@lexer::members {
  ...
  public Token nextToken() {
    ...
    if (_input.LA(1) == EOF && !this.indents.isEmpty()) {
      ...
      this.emit(commonToken(LimeParser.NEWLINE, "\n"));
      ...
      this.emit(commonToken(LimeParser.EOF, "<EOF>"));
    }
  ...
  private Token createDedent() {
    CommonToken dedent = commonToken(LimeParser.DEDENT, "");
   ...
   )
   {
     ...
     if (opened > 0 || next == '\r' || next == '\n' || next == '\f' || next == '#') {
       ...
       if (indent == previous) {
         ...
         indents.push(indent);
         emit(commonToken(LimeParser.INDENT, spaces));
       }
       else {
