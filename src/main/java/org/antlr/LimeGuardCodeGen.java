...
import org.antlr.LimeParser.BlockContext;
import org.antlr.LimeParser.ClassDeclContext;
import org.antlr.LimeParser.ClassMemberContext;
...
import org.antlr.LimeParser.Multi_assignContext;
import org.antlr.LimeParser.Simple_stmtContext;
import org.antlr.LimeParser.Small_stmtContext;
import org.antlr.LimeParser.StmtContext;

public class LimeGuardCodeGen extends LimeBaseVisitor<String>{
    SymbolTable symtab;
	Scope currentScope;
	String guards;
	...
	LimeGuardCodeGen(SymbolTable st) {
		this.symtab = st;
	...
	public String visitMulti_assign(Multi_assignContext ctx) {
		...
		if (ids.length != vals.length) {
			...
			for (int i = 0; i < ids.length; ++i) {
                System.err.printf("handling multi assignment " + ids[i] + " : " + vals[i] + "\n");
	...
	String tmplocalLabel() {
		LimeGuardCodeGen.labelnum++;
		return "" + LimeGuardCodeGen.labelnum;
	}
